...
**Phase 0 is complete** in two languages. Both share the same Postgres database.

| Implementation | Directory | Framework | Driver | Status |
|----------------|-----------|-----------|--------|--------|
| Go | `golang/` | Fiber v2 | pgx v5 | Phase 0 done |
| TypeScript | `expressjs/` | Express 4 | pg (node-postgres) | Phase 0 done |
...
**Admin UI** is in `admin/` — SolidJS + Vite + Tailwind. Covers entity CRUD, relation editing, and data browsing.
...
│   └── internal/
│       ├── config/config.go
│       ├── metadata/{field,entity,relation,registry,loader}.go
│       ├── store/{postgres,bootstrap,migrator}.go
│       ├── engine/{query,errors,soft_delete,writer,diff,nested_write,includes,handler,router}.go
│       └── admin/handler.go
└── expressjs/                   # Express.js implementation
    ...
    └── src/
        ├── index.ts
        ├── config/index.ts
        ├── metadata/{types,registry,loader}.ts
        ├── store/{postgres,bootstrap,migrator,schema}.ts
        ├── engine/{query,errors,soft-delete,writer,diff,nested-write,includes,handler,router}.ts
        ├── admin/handler.ts
...
- Unique constraint violations return 500 instead of 409 CONFLICT (both implementations)
