...
export default function Sidebar() {
  ...
  const [publicPages, setPublicPages] = createSignal<PublicPageLink[]>([]);
  ...
  async function loadEntities() {
    try {
      ...
      // Collect entities that have public pages configured
      const pages: PublicPageLink[] = [];
      for (const e of defs) {
        const uiConfig = getEntityUIConfig(e.name);
        if (uiConfig?.pages?.landing) {
          pages.push({
            entity: e.name,
            label: uiConfig.pages.landing.title ?? uiConfig.sidebar?.label ?? e.name,
            route: `/pages/${e.name}`,
          });
        }
      }
    ...
    <aside class={`sidebar ${sidebarCollapsed() ? "sidebar-collapsed" : ""}`}>
      ...
      <div class="sidebar-scroll-area">
        <nav class="sidebar-nav">
          ...
          <For each={publicPages()}>
            {(page) => (
              <A
                href={page.route}
                class={`nav-link ${isActive(page.route) ? "nav-link-active" : ""}`}
                title={page.label}
              >
                <svg class="nav-link-icon" viewBox="0 0 20 20" fill="currentColor">
