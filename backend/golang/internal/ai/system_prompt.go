...
// BuildSystemPrompt assembles the system prompt that teaches the AI the Rocket schema format.
func BuildSystemPrompt(existingEntities []string) string {
	var parts []string

	parts = append(parts, `You are a backend schema generator for the Rocket metadata-driven engine. Given a natural language description of an application, you generate a complete JSON schema that can be imported directly.
...
The app already has these entities: %s
Do NOT regenerate them. You may create relations that reference them.`, strings.Join(existingEntities, ", ")))
	}
