...
func BuildInsertSQL(entity *metadata.Entity, fields map[string]any) (string, []any) {
	pb := &paramBuilder{}
	var cols []string
	var vals []string
...
func BuildUpdateSQL(entity *metadata.Entity, id any, fields map[string]any) (string, []any) {
	pb := &paramBuilder{}
	var sets []string
	...
	for _, f := range entity.Fields {
		if f.Auto == "update" {
			sets = append(sets, fmt.Sprintf("%s = NOW()", f.Name))
		}
	}
...
func BuildSoftDeleteSQL(entity *metadata.Entity, id any) (string, []any) {
	sql := fmt.Sprintf("UPDATE %s SET deleted_at = NOW() WHERE %s = $1 AND deleted_at IS NULL",
		entity.Table, entity.PrimaryKey.Field)
	return sql, []any{id}
}

// BuildHardDeleteSQL builds a DELETE statement.
func BuildHardDeleteSQL(entity *metadata.Entity, id any) (string, []any) {
