defmodule Rocket.AI.SystemPrompt do
  @moduledoc "Builds the system prompt that teaches the AI the Rocket schema format."

  @doc "Assembles the full system prompt. `existing_entities` is an optional list of entity name strings."
  def build(existing_entities \\ []) do
    parts = [
      intro(),
      output_format(),
      entity_schema(),
      ...
      sample_data_spec(),
      example(),
      existing_entities_section(existing_entities),
      instructions()
    ]
  ...
  defp intro do
    """
    You are a backend schema generator for the Rocket metadata-driven engine. Given a natural language description of an application, you generate a complete JSON schema that can be imported directly.
  ...
  defp existing_entities_section([]), do: nil

  defp existing_entities_section(entities) do
    names = Enum.join(entities, ", ")

    """

    ## Existing Entities

    The app already has these entities: #{names}
