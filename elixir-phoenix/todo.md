...
- [x] Instrumenter behaviour: `start_span(source, component, action) → Span`, `emit_business_event(action, entity, record_id, metadata)`
- [x] Span struct: `finish()`, `set_status()`, `set_metadata()`, `trace_id`, `span_id`
- [x] Trace ID propagation via `Logger.metadata` / process dictionary — generate UUID per request or accept `X-Trace-ID` header
- [x] Auto-instrumented system events:
  - [x] Plug pipeline: request.start, request.end (method, path, status, latency)
  - [x] Auth plug: auth.validate, auth.login, auth.denied
...
- [x] Fire-and-forget event writes via GenServer buffer (handle_cast + Process.send_after flush)
...
## Client App & UI Configs (done)
- [x] `_ui_configs` system table (entity, scope, config JSONB)
- [x] Admin CRUD endpoints (`/_admin/ui-configs`)
