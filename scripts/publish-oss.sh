...
#
# publish-oss.sh — Publish a cleaned copy (no AI artifacts) to the OSS repo.
#
# Usage:
#   ./scripts/publish-oss.sh                          # dry-run (default)
#   ./scripts/publish-oss.sh --push                   # actually push to OSS remote
#   OSS_REMOTE=https://github.com/org/repo.git ./scripts/publish-oss.sh --push
#
...
OSS_REMOTE="${OSS_REMOTE:-https://github.com/algorisys-oss/rocket-framework.git}"
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    warn "  $0 --push"
    warn "  OSS_REMOTE=https://github.com/algorisys-oss/rocket-framework.git $0 --push"
    exit 0
fi

info "Pushing to $OSS_REMOTE ($OSS_BRANCH)"
